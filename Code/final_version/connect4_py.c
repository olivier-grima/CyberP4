...
int update(int coup, char jeton);
...
static int coup_joueur(int coup){
    struct position pos;
...
int ia(void)
{
    ...
    return meilleurs_col[nb_aleatoire_entre(0, nb_meilleurs_col - 1)]+1;
}
...
//fonction appelée en python qui initialise tout
void debutPartie(){
    initialise_grille();
    affiche_grille();
}

//fonction appelée par les callbacks des capteurs optiques
//
int update(int coup, char jeton){
        struct position pos;
        int action;
        ...
        //tour du robot de jouer
        if (/*njoueur == 1 &&*/ jeton == J2_JETON){
            printf("Le robot a joué  dans la colonne: %d\n", coup);
            calcule_position(coup-1, &pos);
            }
        else{
            //action = demande_action(&coup);
            printf("Le joueur a joué la colonne: %d\n", coup);
            calcule_position(coup - 1, &pos);
        }
        ...
        printf("\nAu tour du %s\n", (jeton == J1_JETON) ? "joueur" : "robot");    

        if (statut == STATUT_GAGNE){
            printf("\n-------------------------------------------------------\n");
            printf("%s a gagné\n", (jeton == J1_JETON) ? "Le robot" : "Le joueur");
        }
